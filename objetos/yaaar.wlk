...
class Barco {
    ...
    method esTemible()= self.puedeCompletarMision()
    method puedeCompletarMision()=self.tripulantesSuficientes()
    method tripulantesSuficientes() = tripulantes.length()>=capacidad*90/100
}
class Pirata {
    var items = []
    var property ebriedad 
    var property monedas
    method esUtil()=items.any("brujula") || items.any("mapa") ||items.any("botella de grogXD")
...
//var barbanegra = new Pirata(items=["brujula", "cuchillo", "cuchillo", "diente de oro", "botella de grogXD", "botella de grogXD", "botella de grogXD"], ebriedad=0, monedas=0)
//Por ejemplo: el pirata Barbanegra tiene una brújula, dos cuchillos, un diente de oro y tres botellas de grogXD.

/*
Saber si un pirata es útil para una misión

a) Saber si un pirata puede formar parte de la tripulación de un barco. Esto ocurre cuando el barco tiene lugar para un tripulante más, y además el pirata sirve para la misión del barco.
b) Incorporar un pirata a la tripulación de un barco, sólo si esto puede ser llevado a cabo.
c) Hacer que un barco cambie de misión. Al cambiar de misión, el barco echa a los tripulantes que no sirven para su misión actual.

a) Saber quién es el pirata más ebrio de la tripulación de un barco.
b) Hacer que un barco ancle en una ciudad costera. Cuando los barcos anclan, toda la tripulación  se toma un trago de grogXD, que provoca que el nivel de ebriedad de cada uno suba en 5 unidades y se gasten una moneda. 
...
Cada tripulante conoce qué tripulante del barco lo invito. Se quiere conocer quién es el tripulante de un barco pirata que invito (satisfactoriamente) a más gente.
*/